<template>
  ...
  <ul class="p-2 flex flex-col gap-y-2 h-full overflow-y-auto">
    <li v-for="entry in taskStore.activeTaskDataExplorationTask.value.explorationTaskInteractions" :key="entry.id">
      <div
      ...
      >
        <div class="text-left my-auto truncate">
          <h3 class="font-semibold text-sm">{{ formatTimeStamp(entry.timeStamp) }}</h3>
    ...
    <li
      ...
      v-if="taskStore.activeTaskDataExplorationTask.value.explorationTaskInteractions.length == 0"
    >
...
const formattedDuration = computed(() =>
  formatHelper.formatTimeFromSeconds(taskStore.activeTaskDataExplorationTask.value.timeInSeconds)
);

function updateElapsedTime(): void {
  if (taskStore.activeTaskDataExplorationTask.value === undefined) return;
  taskStore.activeTaskDataExplorationTask.value.updateTimeInSeconds();
...
function handleSubmit() {
  ...
  TaskService.submitExplorationTask(
    taskStore.activeTaskDataExplorationTask.value,
