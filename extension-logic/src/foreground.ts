...
function runScript() {
	...
	// START FIREFOX ONLY!! => Firefox does not allow to send message from a webpage as browser does not exist outside of an extension context
	...
	connection.start().then(() => {
		connection.invoke("JoinWebsiteGroup", windowUrl).then(() => {
			console.log(`Connected to Socket group ${windowUrl}`);
		});
	});
	...
	function markArea(markingType: MarkingType): void {
		...
		// if (!res) {
		// 	console.error("Something went Wrong generating a marking data object");
		// 	return;
		// }
		if (activeTask?.taskType == TaskType.MarkingTask) {
			...
			messageService
				.pushMarkingData(res)
				.then((res) => console.log(`Was successful: ${res}`))
	...
	Browser.runtime.onMessage.addListener((request) => {
		console.log(`Received Message Foreground: ${request.message} with data: ${JSON.stringify(request.data, null, 2)}`);
